...
const dbFilesHelp = {
	...
	getFilesAndDirsOfParentIndex: (req, res) => {
		...
		//pgadmin 语句
		// 		SELECT * FROM(
		// 			SELECT id, parentindex, name, permissions, haspermissions, null as upuser, null as size, null as path, null as mimetype, null as md5, 'dir' as type FROM public.directorys where parentindex = 1  
		//         union
		//           SELECT id, dirindex as parentindex, originalname as name, null as permissions, null as haspermissions, upuser, size, path, mimetype, md5, 'file' as type  FROM public.files where dirindex = 1
		// 		)
		//         AS foo 
		//         ORDER BY type, id

		db.any('SELECT  * FROM ( SELECT id, parentindex, name, permissions,haspermissions,null as upuser, null as size, null as path,null as mimetype, null as md5, $1 as type FROM public.directorys where parentindex  =$3  union SELECT id, dirindex as parentindex,  originalname as name, null as permissions, null as haspermissions ,upuser, size, path, mimetype,md5, $2 as type  FROM public.files where dirindex  = $3) AS foo ORDER BY type , id', [typeDir, typefile, parentIndex])
